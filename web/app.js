...
const copy = {
  en: {
    ...
    productDescriptor: 'Hybrid Social Intelligence', productNav: 'Product navigation', evidenceNav: 'Evidence', allocationLab: 'Allocation Lab', technical: 'Technical', researchPrototype: 'Research prototype', heroKicker: 'Context before reaction', heroTitle: 'A social feed that can trace evidence and route human attention.', heroText: 'DRSK connects SOURCECHAIN verification with NIYET coordination—without hiding uncertainty or presenting development signals as probabilities.', systemLayers: 'DRSK system layers', verifyClaims: 'Verify claims', routeAttention: 'Route attention',
    placeholder: 'What is happening?', publish: 'Post', demoHelp: 'Try: ask for help', demoCollab: 'Try: find a collaborator', demoClaim: 'Try: check claim & distortions', demoNormal: 'Try: normal post',
    ...
    previewTitle: 'Responder preview', previewSub: 'What a routed request looks like on the other side', live: 'live', waiting: 'WAITING', noRoute: 'no route yet', previewEmpty: 'Route a response-seeking post to see the responder side.',
    ...
    responderSide: 'Responder preview', authorSide: 'Author side', openResponder: 'Open responder view', close: 'Close', reset: 'Reset demo', resetToast: 'Demo session reset',
  ...
  tr: {
    ...
    productDescriptor: 'Hibrit Sosyal Zeka', productNav: 'Ürün gezinmesi', evidenceNav: 'Kanıt', allocationLab: 'Dağıtım Laboratuvarı', technical: 'Teknik', researchPrototype: 'Araştırma prototipi', heroKicker: 'Tepkiden önce bağlam', heroTitle: 'Kanıtın izini süren ve insan dikkatini yönlendiren bir sosyal akış.', heroText: 'DRSK, SOURCECHAIN doğrulamasıyla NIYET koordinasyonunu birleştirir; belirsizliği saklamaz ve geliştirme sinyallerini olasılık gibi sunmaz.', systemLayers: 'DRSK sistem katmanları', verifyClaims: 'İddiaları doğrula', routeAttention: 'Dikkati yönlendir',
    placeholder: 'Neler oluyor?', publish: 'Gönder', demoHelp: 'Dene: yardım iste', demoCollab: 'Dene: ekip arkadaşı bul', demoClaim: 'Dene: iddia ve bozulma kontrolü', demoNormal: 'Dene: normal gönderi',
    ...
    responderSide: 'Cevaplayıcı önizlemesi', authorSide: 'Gönderi sahibi', openResponder: 'Cevaplayıcı görünümünü aç', close: 'Kapat', reset: 'Demoyu sıfırla', resetToast: 'Demo oturumu sıfırlandı',
    ...
    primaryNav: 'Ana gezinme', languageLabel: 'Dil', demoPrompts: 'Demo örnekleri', postTools: 'Gönderi araçları', media: 'Medya', poll: 'Anket', location: 'Konum', contextSidebar: 'Bağlam kenar çubuğu', mobileNav: 'Mobil gezinme', reply: 'Yanıtla', repost: 'Yeniden gönder', like: 'Beğen', share: 'Paylaş', toggleRouting: 'NIYET yönlendirmesini aç veya kapat', notRun: 'çalıştırılmadı', skipToFeed: 'Akışa geç',
    now: 'şimdi', claimVsSource: 'İddia ve Kaynak Karşılaştırması', postClaims: 'Gönderi iddiası', sourceStates: 'Kaynak ifadesi', shiftDetected: 'Sapma tespit edildi'
  }
};
...
function localNeedsResponse(value) {
  ...
  return /\?|yardım|nasıl|neden|sence|sizce|arıyorum|katıl|öner|help|feedback|looking for|collab|who wants/i.test(value.toLocaleLowerCase('tr-TR'));
}

function localIntent(value) {
  const lower = value.toLocaleLowerCase('tr-TR');
  if (/(ekip|birlikte|arıyorum|collab|teammate|looking for|collaborat)/i.test(lower)) return 'collaborate';
...
function displayCode(value) {
  ...
  const labels = language === 'tr' ? {
    ...
    CONFLICTING: 'conflicting', INSUFFICIENT: 'insufficient', PARTIAL: 'partial',
    EVIDENCE: 'evidence', HUMAN: 'human', BOTH: 'evidence + human', NONE: 'none needed', DEFERRED: 'deferred',
    NUMERIC_SHIFT: 'numeric shift', CAUSALITY_SHIFT: 'causality shift',
    CERTAINTY_SHIFT: 'certainty shift', ATTRIBUTION_SHIFT: 'attribution shift',
    SOURCE_MISMATCH: 'source mismatch'
  };
  return labels[code] || code.replaceAll('_', ' ').toLowerCase();
}

function localizeExplanation(rawText) {
  if (language !== 'tr' || !rawText) return rawText;
  let out = rawText;
  out = out.replace('The controlled evidence supports only part of the claim.', 'Kontrollü kanıt iddianın yalnızca bir kısmını destekliyor.');
  out = out.replace('The controlled evidence supports the claim.', 'Kontrollü kanıt iddiayı destekliyor.');
  out = out.replace('The controlled evidence contradicts the claim.', 'Kontrollü kanıt iddiayla çelişiyor.');
  out = out.replace('No sufficiently relevant passage was found in the controlled evidence corpus.', 'Kontrollü kanıt derleminde yeterince ilgili bir bölüm bulunamadı.');
  out = out.replace('No factual claim was identified that requires verification.', 'Doğrulama gerektiren olgusal bir iddia bulunamadı.');
  return out;
}
...
function renderEvidence(payload) {
  ...
  const rawExpl = status === 'NOT_REQUIRED' ? text('evidenceNotRequired') : (bundle.explanation || (evidence.length ? '' : text('noEvidence')));
  $('#evidenceExplanation').textContent = localizeExplanation(rawExpl);
  ...
  evidence.forEach((item) => {
    ...
    // Distortion Lens & Source Mismatch Comparison Component
    if (item.passage || claimText) {
      ...
      const shiftText = distortions.length ? distortions.map(displayCode).join(' + ') : displayCode(item.relation);
      appendTextElement(transform, 'span', '', shiftText);
      transform.setAttribute('aria-label', `${text('distortionLabel')}: ${shiftText}`);
...
function resetEvidence() {
  ...
  const label = $('.evidence-toggle-label');
  if (label) label.textContent = text('showEvidence');
}
...
function renderMatchPreview(decision) {
  ...
  if (!decision?.match) {
    ...
    $('#matchPostText').textContent = $('#composerText')?.value.trim() || text('previewEmpty');
...
function updateBudget() {
  ...
  const fill = $('#budgetFill');
  if (fill) fill.style.width = `${Math.min(100, Math.max(0, (remaining / daily) * 100))}%`;
}
...
function demoExamples() {
  if (language === 'tr') return {
    ...
    claim: 'Coffee consumption causes a 50% decrease in total mortality according to WHO.',
...
function activateView(view) {
  ...
  if (composer) composer.hidden = !isFeed;
  const feedPosts = $('#feedPosts');
  if (feedPosts) feedPosts.hidden = !isFeed;
  const feedTabs = $('.feed-tabs');
  if (feedTabs) feedTabs.hidden = !isFeed;
  const feedTitle = $('.feed-title');
  if (feedTitle) feedTitle.textContent = isFeed ? text('feed') : text(`view${view[0].toUpperCase()}${view.slice(1)}Title`);
  renderSecondaryView();
...
function bindEvents() {
  ...
  if (textarea) {
    textarea.addEventListener('input', () => {
      ...
      analyzeTimer = setTimeout(analyzePost, 600);
    });
  }
  ...
  $('#evidenceToggle')?.addEventListener('click', () => {
    ...
    const label = $('.evidence-toggle-label');
    if (label) label.textContent = text(expanded ? 'hideEvidence' : 'showEvidence');
  });
  $('#askPerson')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const label = $('.evidence-toggle-label');
      if (label) label.textContent = text('hideEvidence');
    } catch (_) {
      if (button) {
        button.disabled = false;
