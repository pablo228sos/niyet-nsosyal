...
- The service worker accepts only `resolve` and `status`, validates the payload, and calls only `https://niyet-nsosyal.vercel.app/api/human-help` over HTTPS.
...
The extension asks only for the NSosyal page access needed by its content script and access to the fixed DRSK backend host. It does not request storage, broad web access, or NSosyal API credentials.
