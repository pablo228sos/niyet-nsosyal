...
class NiyetService:
    ...
    def _allocate(
        ...
        *,
        ...
        query_tokens = set(re.findall(r"[\w+#.-]+", request["text"].lower()))
        matches: list[CandidateMatch] = []
        responders: list[Responder] = []
        relevance_by_uid: dict[str, float] = {}
        for profile in profiles:
            uid = profile["uid"]
            if uid in excluded or uid == request["author_uid"]:
                continue
            topic_tokens = set(re.findall(r"[\w+#.-]+", " ".join(profile.get("topics", [])).lower()))
            ...
            if available <= 0:
                continue
            responders.append(Responder(uid, tuple(profile.get("topics", [])), (), available, True))
            match = CandidateMatch(request["id"], uid, min(1.0, relevance), 1.0, available / max(1, int(profile["capacity_total"])))
            matches.append(match)
            relevance_by_uid[uid] = relevance
        assignments = global_allocate(matches, responders, min_score=0.0)
        ranked = sorted(assignments, key=lambda item: item.score, reverse=True)
        for candidate in ranked:
            metadata = {"relevance": relevance_by_uid[candidate.responder_id], "score": candidate.score, "strategy": "niyet_global_allocate"}
            if reallocated_from_assignment_id:
                metadata["reallocated_from_assignment_id"] = reallocated_from_assignment_id
